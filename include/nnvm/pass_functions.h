...
inline Graph SplitDistributedGraph(
    ...
    const std::string& p2pnet_send_sink_op, size_t* num_forward_inputs,
    size_t* num_forward_outputs) {
  graph.attrs["address"] = std::make_shared<any>(std::move(address_vec));
  graph.attrs["localhost"] = std::make_shared<any>(std::move(localhost));
  graph.attrs["shape"] = std::make_shared<any>(std::move(shape_vec));
  graph.attrs["dtype"] = std::make_shared<any>(std::move(dtype_vec));
  graph.attrs["device_copy_op"]
      = std::make_shared<any>(std::move(device_copy_op));
  graph.attrs["p2pnet_init_op"]
      = std::make_shared<any>(std::move(p2pnet_init_op));
  graph.attrs["p2pnet_send_op"]
      = std::make_shared<any>(std::move(p2pnet_send_op));
  graph.attrs["p2pnet_recv_op"]
      = std::make_shared<any>(std::move(p2pnet_recv_op));
  graph.attrs["p2pnet_send_sink_op"]
      = std::make_shared<any>(std::move(p2pnet_send_sink_op));
  graph.attrs["num_forward_inputs"]
      = std::make_shared<any>(*num_forward_inputs);
  graph.attrs["num_forward_outputs"]
      = std::make_shared<any>(*num_forward_outputs);
